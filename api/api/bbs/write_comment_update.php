...
trait write_comment_update {
  public function write_comment_update($bo_table, $wr_id, $w) {
    ...
    $wr_facebook_user = isset($_POST['wr_facebook_user']) ? $this->clean_xss_tags($_POST['wr_facebook_user'], 1, 1) : '';
    $wr_twitter_user = isset($_POST['wr_twitter_user']) ? $this->clean_xss_tags($_POST['wr_twitter_user'], 1, 1) : '';
    $wr_homepage = isset($_POST['wr_homepage']) ? $this->clean_xss_tags($_POST['wr_homepage'], 1, 1) : '';
    ...
    if ($w == 'c') {// 댓글 입력
      ...
      if ($tmp_point + $board['bo_comment_point'] < 0 && !$is_admin)
        $this->alert('보유하신 포인트('.number_format($member['mb_point']).')가 없거나 모자라서 댓글쓰기('.number_format($board['bo_comment_point']).')가 불가합니다.\\n\\n포인트를 적립하신 후 다시 댓글을 써 주십시오.');

      // 댓글 답변
      if ($comment_id) {
        $reply_array = $this->get_write($write_table, $comment_id, true);
        if (!$reply_array['wr_id'])
          $this->alert('답변할 댓글이 없습니다.\\n\\n답변하는 동안 댓글이 삭제되었을 수 있습니다.');

        if($wr['wr_parent'] != $reply_array['wr_parent'])
          $this->alert('댓글을 등록할 수 없습니다.');
      ...
      if ($config['cf_email_use'] && $board['bo_use_email']) {
        ...
        $wr_content = nl2br($this->get_text(stripslashes("원글\n{$wr['wr_subject']}\n\n\n댓글\n$wr_content")));
        ...
        $link_url = $this->get_pretty_url($bo_table, $wr_id, $qstr."#c_".$comment_id);
    ...
    */
    ...
    if ($is_admin == 'super') { // 최고관리자 통과
      ...
      $mb = $this->get_member($comment['mb_id']);
      if ($member['mb_id'] === $group['gr_admin']) { // 자신이 관리하는 그룹인가?
        if ($member['mb_level'] >= $mb['mb_level']) {// 자신의 레벨이 크거나 같다면 통과
          ;
        } else {
          $this->alert('그룹관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.');
        }
      } else {
        $this->alert('자신이 관리하는 그룹의 게시판이 아니므로 댓글을 수정할 수 없습니다.');
      }
      ...
      if ($member['mb_id'] === $board['bo_admin']) { // 자신이 관리하는 게시판인가?
        if ($member['mb_level'] >= $mb['mb_level']) {// 자신의 레벨이 크거나 같다면 통과
          ...
          $this->alert('게시판관리자의 권한보다 높은 회원의 댓글이므로 수정할 수 없습니다.');
        }
      } else
        $this->alert('자신이 관리하는 게시판이 아니므로 댓글을 수정할 수 없습니다.');
    } else if ($member['mb_id']) {
      if ($member['mb_id'] !== $comment['mb_id'])
        $this->alert('자신의 글이 아니므로 수정할 수 없습니다.');
    } else {
      if( !($comment['mb_id'] === '' && $comment['wr_password'] && $this->check_password($post_wr_password, $comment['wr_password'])) )
        $this->alert('댓글을 수정할 권한이 없습니다.');
    }
