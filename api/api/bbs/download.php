...
trait download {
  public function download ($no) {
    ...
    if (!$this->get_session('ss_view_'.$bo_table.'_'.$wr_id))
      $this->alert('잘못된 접근입니다.');

    // 다운로드 차감일 때 비회원은 다운로드 불가
    if($board['bo_download_point'] < 0 && $is_guest)
      $this->alert('다운로드 권한이 없습니다.\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login.php?wr_id='.$wr_id.'&amp;'.$qstr.'&amp;url='.urlencode($this->get_pretty_url($bo_table, $wr_id)));

    $sql = " select * from {$g5['board_file_table']} where bo_table = ? and wr_id = ? and bf_no = ? ";
    $file = sql_fetch($sql, [$bo_table, $wr_id, $no]);
    if (!$file['bf_file'])
      $this->alert('파일 정보가 존재하지 않습니다.');

    if ($member['mb_level'] < $board['bo_download_level']) {
      $alert_msg = '다운로드 권한이 없습니다.';
      if ($member['mb_id'])
        $this->alert($alert_msg);
      else
        $this->alert($alert_msg.'\\n회원이시라면 로그인 후 이용해 보십시오.', G5_BBS_URL.'/login.php?wr_id='.$wr_id.'&amp;'.$qstr.'&amp;url='.urlencode($this->get_pretty_url($bo_table, $wr_id)));
    }
    ...
    if (!$this->get_session($ss_name)) {
      ...
      if (($write['mb_id'] && $write['mb_id'] == $member['mb_id']) || $is_admin) {
        ...
        if ($member['mb_point'] + $board['bo_download_point'] < 0)
          $this->alert('보유하신 포인트('.number_format($member['mb_point']).')가 없거나 모자라서 다운로드('.number_format($board['bo_download_point']).')가 불가합니다.\\n\\n포인트를 적립하신 후 다시 다운로드 해 주십시오.');

        // 게시물당 한번만 차감하도록 수정
        $this->insert_point($member['mb_id'], $board['bo_download_point'], "{$board['bo_subject']} $wr_id 파일 다운로드", $bo_table, $wr_id, "다운로드");
      }
    ...
    if (!$this->get_session($ss_name)) {
      // 다운로드 카운트 증가
      $sql = " update {$g5['board_file_table']} set bf_download = bf_download + 1 where bo_table = ? and wr_id = ? and bf_no = ?";
      $this->sql_query($sql, [$bo_table, $wr_id, $no]);
