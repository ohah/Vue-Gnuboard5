...
trait thumbnaillib {
  ...
  public function get_file_thumbnail($file){  
    ...
    if( preg_match('/(\.jpg|\.jpeg|\.gif|\.png|\.bmp)$/i', $file['file']) && $contents = run_replace('get_file_thumbnail_tags', '', $file) ){
      ...
      return $this->get_view_thumbnail($file['view']);
    }
  ...
  // 게시글보기 썸네일 생성
  public function get_view_thumbnail($contents, $thumb_width=0) {
    global $board;
    $config = $this->config;
    if (!$thumb_width)
    ...
    for($i=0; $i<count($matches[1]); $i++) {
      ...
      if(is_file($srcfile)) {
        ...
        $is_animated = false;
        if($size[2] == 1) {
          $is_animated = $this->is_animated_gif($srcfile);
  ...
  public function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3') {
    global $g5;
